...
pT_yy_1.pdf: fit.so draw.so Makefile $(shell which hed)
	hed H1j_NLO.root H1j_B.root H1j_B_mtop.root AA1j_NLO.root -o $@ \
	  ...
	  -g 'margin 0.1:0.1:0.04:0.1' 'rat 0.3' \
	     'log y' 'leg 0.72:0.68:0.96:0.9' \
	     't//Diphoton pT @ NLO + #gamma cuts/' \
	     'x//#gamma#gamma p_{T} [GeV]/' 'y,,d#sigma/dp_{T}  [fb/GeV],' \
	     'title_offset x 1.25' 'title_offset y 0.7' \
	     'tex 750:0.03 H#rightarrow#gamma#gamma#kern[0.4]{B}R:#kern[0.4]{2}.27#times10^{#minus3} 12 1' \
	     'load ./draw.so' \
	     'tex 60:1.3e-4 y=exp(A+Blogx+Clog^{2}x) 12 1.0' \
	     'pad 2' 'log y' \
	  --colors 602 41 44 46 8 94 -vc
